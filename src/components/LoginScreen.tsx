...
export default function LoginScreen({ onLogin, members }: LoginScreenProps) {
  ...
  const handleGoogleLogin = (e: React.FormEvent) => {
    ...
    onLogin({ id: '', name, email, avatar: picture, class: '', level: 0, rank: 'Recruit', role: 'member', points: 0, joinedAt: '' } as Member)
      .then(() => {
        // Login succeeded
      })
      .catch(() => {
        // Not registered - show registration form
        setAuthenticatedGoogleUser({ email, name, picture });
        if (activeInvite) {
  ...
  const handeRegisterSubmit = (e: React.FormEvent) => {
    ...
    // The backend will consume the invite automatically when the user logs in with this email
    onLogin(newMember);
  };
          ...
          {isLoadingAuth ? (
            ...
            >
              ...
              <form onSubmit={handleGoogleLogin} className="space-y-3">
                <div>
                  <label className="block text-[10px] font-mono text-slate-400 uppercase tracking-widest mb-1.5">Google Email</label>
                  <input
                    type="email"
                    required
                    value={loginEmail}
                    ...
                    className="w-full h-11 px-4 bg-[#08090d] border border-slate-800 focus:border-cyan-500/50 rounded-xl text-slate-200 text-sm focus:outline-none"
                  />
                </div>
                <button
                  type="submit"
                  disabled={isLoadingAuth}
                  className="w-full h-13 bg-white hover:bg-zinc-100 text-[#1f1f1f] font-bold text-xs sm:text-sm uppercase tracking-wider rounded-xl transition-all cursor-pointer flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(255,255,255,0.05)] hover:shadow-[0_0_25px_rgba(255,255,255,0.12)] active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  ...
                  {isLoadingAuth ? 'Authenticating...' : 'Sign in with Google'}
                </button>
              </form>
